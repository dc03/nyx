...
struct ParseException : public std::invalid_argument {
    ...
    explicit ParseException(Token token, const std::string_view error)
        : std::invalid_argument(std::string { error.begin(), error.end() })
...
template <typename T>
inline Parser<T>::Parser(const std::vector<Token> &tokens)
    : tokens { tokens } {
...
inline expr_node_t<T> Parser<T>::variable(bool can_assign) {
    if (can_assign && match(TokenType::EQUAL, TokenType::PLUS_EQUAL, TokenType::MINUS_EQUAL, TokenType::STAR_EQUAL, TokenType::SLASH_EQUAL)) {
            ...
            if (method_name.lexeme == name.lexeme) {
                if (found_dtor && !has_dtor) {
                    ...
                    case VisibilityType::PUBLIC:
                        visibility = VisibilityType::PUBLIC_DTOR;
                        break;
                    case VisibilityType::PRIVATE:
                        visibility = VisibilityType::PRIVATE_DTOR;
                        break;
                    case VisibilityType::PROTECTED:
                        visibility = VisibilityType::PROTECTED_DTOR;
                        break;
